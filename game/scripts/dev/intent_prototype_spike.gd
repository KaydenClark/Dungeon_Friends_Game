...
var cue_plays := 0
## T-096/T-097 seam: Sol's neutral formation/deployment snapshot, when
## available. Empty until the branches merge; see _apply_deployment_snapshot.
...
## Sol seam (T-096 -> T-097): reposition party members onto the planner's
## deployment cells when a snapshot is present. All formation/fallback logic
## stays on Sol's side; this only applies the neutral output.
func _apply_deployment_snapshot() -> void:
	...
	for id: String in starts:
		...
		if unit == null or unit.cell == c or not room.is_walkable(c):
			continue
		room.move_occupant(unit, unit.cell, c)
		unit.cell = c
...
func _scripted_tour() -> void:
	...
	await _encounter_step("blocker1", Vector2i.LEFT)
	_assert((followers[1] as GridActor).cell == Vector2i(7, 3),
			"blocker1 moved first, out of roster order")
	await _encounter_step("blocker2", Vector2i.UP)
	_assert((followers[2] as GridActor).cell == Vector2i(8, 3),
			"blocker2 moved into the vacated cell")
	...
	# 5) Guard vs the line: blocker1 turns to face it and raises a guard.
	await _encounter_step("blocker1", Vector2i.UP)  # bump on the hero: turn only
	_assert((followers[1] as GridActor).cell == Vector2i(7, 3)
			and (followers[1] as GridActor).facing == Vector2i.UP,
			"a blocked step turns the guard toward the line without moving")
	_switch_unit()  # hero -> friend
	_switch_unit()  # friend -> blocker1
	_assert(active_id == "blocker1", "tab cycling reaches every open member")
	_select_ability("guard")
	await _commit_ability()
	...
	# 7) Target invalidation: blocker1 is now nearest, so the old hero plan
	#    rebuilt from scratch instead of continuing with its stale slam.
	_assert(current_verb == "move",
			"the invalidated plan rebuilt (a stale continuation would slam)")
	_assert(str(plan[0]["target_id"]) == "blocker1",
