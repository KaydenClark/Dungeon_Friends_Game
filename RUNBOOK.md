...
victory-continuous) and prints 43 `PASS:` assertions, exiting `0`; any
`FAIL:` exits `1`. The pure rules live in `game/scripts/dev/intent_logic.gd`
with red/green coverage in `tests/test_intent_logic.gd`; Sol's T-096
deployment snapshot enters through the tiny
`game/scripts/dev/sol_snapshot_adapter.gd` seam. **D-027 is resolved in
