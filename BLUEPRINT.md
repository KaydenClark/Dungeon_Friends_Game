...
| [S-004 - V2 Thesis Slice](specs/S-004-thesis-slice/SPEC.md) | Build one authored recruit, non-combat resolution, shared-vocabulary puzzle, tactical fight, and persistent world change as a playable v2 loop. | active |
...
| **D-043 (flagged per D-038) the S-013 progression fork, resolved from D-028/D-033 canon.** (1) **Defeat penalty:** the D-014 25% above-floor XP loss is retired - under finite XP a loss-based penalty can permanently soft-lock required growth. Defeat keeps XP and inventory; the cost is the T-041 checkpoint reposition at 80% HP (D-015 unchanged), and because resolved encounters stay resolved there is no re-fight toll. This closes D-028's flagged D-008 review. (2) **Revive/downed members:** an ally at 0 HP is KO'd for the rest of the encounter and self-revives at 1 HP when it ends either way; full defeat fires only when every active member is down; healer/crystal full-heals unchanged. No permadeath, no revival consumables at thesis scope. (3) **Currency:** none in the thesis slice - rewards flow from treasure, quests, and resolved problems (D-028); coin arrives with the first shop at Steam-demo scope. (4) **Equipment:** the existing item-flag pattern (shield -> guard access) is the whole thesis equipment surface; no slots or loadout UI. (5) **Real recruit:** one authored Dungeon Friend with a primary world verb from the accepted reaction vocabulary, a small deterministic kit, one passive, and a stable save identity replaces the temporary Buddy as the D-033 thesis recruit; the companion_test contract does not graduate by inertia. | S-013/TK-001 required these five calls before implementation; each follows the tightest reading of D-028's finite economy, D-033's roster scope, and the D-014/D-015 checkpoint feel Kayden already tuned. Agent product call under D-038; balance verdict batches at the S-004 replay. | 2026-07-19 / S-013 TK-001 (claude-engineer, flagged) |
...
| **D-044 (flagged per D-038) the thesis route: Wren and the Withered Grove.** S-004's playable loop is one new authored room (`grove.ldtk`, map id `withered_grove`) off the forest's south edge: Wren - the S-013 recruit data contract - is placed as the forest's Growth friend and joins on dialogue; Moss the forager's withered herb bed is the non-combat resolution (grow the bed cell); a vine gate keyed to a grown trellis cell is the shared-vocabulary puzzle; one authored `grove_guardian` slime is the tactical fight; victory regrows the grove heart and everything (recruit, flags, gate, bed, heart, resolved encounter) persists per S-003. Buddy stays the D-013 pair member; Wren joins as the roster-driven third (D-040). Full contract: `docs/planning/S004_THESIS_ROUTE.md` | The shortest route that exercises every thesis beat with already-proven contracts, adding only five small mechanics (grove room, recruit-on-dialogue, watched-cell NPC resolution, vine gate, victory regrowth). A new room keeps the smoke-pinned v1 forest slice intact; the D-035 local-consequence framing (the dragon's shadow withers the forest) recruits the friend whose verb undoes it. Agent product call under D-038; batch review at S-004 TK-004. | 2026-07-20 / S-004 TK-001 (claude-engineer, flagged) |
